...
class ICPManage(Resource):
    ...
    def __init__(self):
        ...
        self.parser.add_argument("serviceId", type=str)
    ...
    def get(self):
        ...
        if key_searchParams:
            ...
            if 'keyword' in search_dict:
                paginate1 = ICP.query.filter(or_(getattr(ICP,_).like("%" + search_dict['keyword'] + "%") for _ in ['task_name']))
        else:
            paginate1 = ICP.query.filter()
        jsondata.update({'count': paginate1.count()})
        paginate1 = paginate1.order_by(ICP.create_time.desc())
        ...
        if paginate:
            ...
            for i in paginate:
                data1 = {}
                data1['id'] = index
                data1['serviceId'] = i.serviceId
                data1['domain'] = i.domain
                data1['serviceLicence'] = i.serviceLicence
                data1['updateRecordTime'] = i.updateRecordTime
                data1['status'] = i.status
                data1['unitName'] = i.unitName
    ...
    def delete(self):
        '''删除'''
        args = self.parser.parse_args()
        key_serviceId = args.serviceId
        _query = ICP.query.filter(ICP.serviceId == key_serviceId).first()
        if not _query:
            return message(202, '')
