...
from web import db
from web.models import GithubRepos
from web.utils.common import login_required, message
from sqlalchemy import or_


class GithubReposManage(Resource):
    '''其他管理'''
    ...
    def get(self):
        ...
        if key_searchParams:
            ...
            if 'keyword' in search_dict:
                paginate1 = GithubRepos.query.filter(or_(getattr(GithubRepos,_).like("%" + search_dict['keyword'] + "%") for _ in ['task_name']))
        else:
            paginate1 = GithubRepos.query.filter()
        jsondata.update({'count': paginate1.count()})
        paginate1 = paginate1.order_by(GithubRepos.create_time.desc())
        ...
        if paginate:
            ...
            for i in paginate:
                ...
                data1['html_url'] = i.html_url
                data1['description'] = i.description
                data1['updated_at'] = i.updated_at
    ...
    def delete(self):
        ...
        _query = GithubRepos.query.filter(GithubRepos.task_name == key_name).first()
        if not _query:
            return message(202, '')
