...
def init():
    ...
    if not job1:
        scheduler.add_job(func=update_sub_task, id='update_sub_task', trigger='cron',
                          second="0", minute="*/2", replace_existing=True, timezone='Asia/Shanghai')
    job2 = scheduler.get_job('update_sub_task_result')
    if not job2:
        scheduler.add_job(func=update_sub_task_result, id='update_sub_task_result', trigger='cron',
                          second="0", minute="*/2", replace_existing=True, timezone='Asia/Shanghai')
